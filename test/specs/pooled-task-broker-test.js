...
test('should authenticate with `Pooled Broker Config`', function(t) {
    ...
    pBroker = rbroker.pooledTaskBroker(brokerConfig)
        .complete(function(rTask, rTaskResult) {
            ...
            //throw new Error('Error in .complete callback....')
        })
        .error(function(err) {
            ...
            console.log(err)
                //console.log('2 Done:: ' + rTaskToken.isDone())
            console.log('2 Pending:: ' + rTaskToken.isPending())
            console.log('    2 End:: ' + token.isDone())
            //token.cancel(false);
        })
        .idle(function() {
            console.log('idle......')
            pBroker.shutdown().then(function() {
                ...
                console.log('5 Pending:: ' + rTaskToken.isPending())
                console.log('    5 End:: ' + token.isDone())
            }, function(err) {
    ...
    var rTask = rbroker.pooledTask({
        Afilename: 'Histogram of Auto Sales',
        directory: 'root',
        author: 'testuser'
    });

    var token = pBroker.submit(rTask);


    var rTaskToken = token.promise()
        .then(function(res) {
            var rTask = res.task,
        ...
        .error(function(err) {
            ...
            console.log('TASK:: ' + err.task.directory)
            console.log('TASK:: ' + err.task.directory)
                //console.log('3 Done:: ' + rTaskToken.isDone())
            console.log('3 Pending:: ' + rTaskToken.isPending())
            console.log('    3 End:: ' + token.isDone())
            console.log('---------------------------------------------------');
        })
        .ensure(function() {
            console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Individual TASK ensure');
            //console.log(rTask7);   
            console.log('4 Pending:: ' + rTaskToken.isPending())
            console.log('    4 End:: ' + token.isDone())
            console.log('----------------------------------------------------');
        }).then(function() {
            console.log('then this 1')
        });

    console.log('Pending:: ' + rTaskToken.isPending())
    console.log('    End:: ' + token.isDone())
      token.cancel(false);      
